...
def calculate():
    ...
    acceptableWeightList = [mhighWeightList[age], mlowWeightList[age]]
    


    # Point system to determine severity based on age, weight, and virion count
    severity = (1-age/24)*(virionCount)**2 + abs((mean(acceptableWeightList) - bmi)/(mean(acceptableWeightList))*(virionCount))
    severityPoints = 0
    ...
    if severityPoints == 3:
        infectabilityReport = "High: " + str(severity)
    elif severityPoints == 2:
        infectabilityReport = "Moderate: " + str(severity)
    elif severityPoints == 1 or severityPoints == 0 or severityPoints == -1:
        infectabilityReport = "Low: " + str(severity)
